...
public class Game extends Canvas implements Runnable {
	...
	public void run() {
		...
		double nsPerTick = 1000000000.0 / 60; //ns per frame @60FPS
		int frames = 0;
		int ticks = 0;
		long lastTimer1 = System.currentTimeMillis();

		init(); //create titlemenu, create&place player in level 3

		while (running) {
			long now = System.nanoTime();
			//number of world updates between now and lasttime
			unprocessed += (now - lastTime) / nsPerTick;
			lastTime = now;
			boolean shouldRender = true;
			//update world through processing, about 60x per second
			while (unprocessed >= 1) {
				ticks++;
				tick(); //process a frame and decrement
				unprocessed -= 1;
				shouldRender = true;
			}
			...
			//display frame, appears unbounded
			if (shouldRender) {
				frames++;
				render();
			}
			//display the number of ticks & frames each second
	...
	public void tick() {
		tickCount++;
		//is the game in focus? Frame selected
		if (!hasFocus()) {
			input.releaseAll();
		} else {
			//advance game time while playing
			if (!player.removed && !hasWon) gameTime++;
			//update player input tick
			input.tick();
			if (menu != null) {
				//update menu with its tick
				menu.tick();
			} else {
				//if player is dead wait 60 then display dead menu
				if (player.removed) {
					playerDeadTime++;
				...
				//wait for a while then diplay won menu if won
				if (wonTimer > 0) {
					if (--wonTimer == 0) {
						setMenu(new WonMenu());
					}
	...
	public void render() {
		...
		g.dispose();
		//displays the screen, uses buffered images 1-3
		bs.show();
	}
