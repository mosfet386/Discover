...
public class Board extends JPanel implements ActionListener {
	...
	public Board(JavaTetris parent) {
		setFocusable(true);
		curPiece=new Shape();
	...
	private void pause() {
		if(!isStarted) {return;}
		isPaused = !isPaused;
		if(isPaused) {timer.stop(); statusbar.setText("Paused");}
		else{timer.start(); statusbar.setText(String.valueOf(numLinesRemoved));}
		repaint();
	}
	...
	private void clearBoard() {
		for(int i=0;i<BoardHeight*BoardWidth;++i){board[i]=Shape.Tetrominoes.NoShape;}
	}
	...
	private void removeFullLines() {
		...
		for(int i=BoardHeight-1;i>=0;--i){
			...
			for(int j=0;j<BoardWidth;++j)
				if(shapeAt(j,i)==Shape.Tetrominoes.NoShape){lineIsFull=false;break;}
			if(lineIsFull){
				++numFullLines;
